...
#' @return
#'
#' 'squash', 'iotools', 'seqminer', 'copynumber'
#' remotes::install_github("ShixiangWang/copynumber")

suppressPackageStartupMessages(library(gdata))
suppressPackageStartupMessages(library(limma))
suppressPackageStartupMessages(library(sequenza)) # BiocManager::install("igordot/copynumber") remotes::install_github("ShixiangWang/copynumber").0
clonality.estimation <- function(mutation.table.loc,
  ...
  # 20230727 update by wenchuan.xie
  # record with absolute.ccf.0.95 == NA, was preprocess !
  # library(magrittr)
  # TCGA.earlyLate.out <- TCGA.earlyLate.out %>%
  #   dplyr::mutate(comb.timing = case_when(is.na(absolute.ccf.0.95)~ comb.timing,
  #                                         absolute.ccf.0.95 >= sub.clonal.cut.off & !timing %in% c("late") ~ "Early",
  #                                         absolute.ccf.0.95 < sub.clonal.cut.off | timing %in% c("late") ~ "Late",
  #                                         TRUE ~ comb.timing))
